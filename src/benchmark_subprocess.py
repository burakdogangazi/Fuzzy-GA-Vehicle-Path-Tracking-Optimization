...
class BenchmarkRunner:
    ...
    def export_all_reports(self):
        """Export all 4 CSV reports"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # 1. Detailed runs
        self._export_detailed_runs(timestamp)
        
        # 2. Strategy comparison
        self._export_strategy_comparison(timestamp)
        
        # 3. Path comparison
        self._export_path_comparison(timestamp)
        
        # 4. Summary statistics
        self._export_summary_statistics(timestamp)
        
        self.print_console_summary()
    
    def _export_detailed_runs(self, timestamp):
        """Export detailed metrics for each run"""
        csv_path = os.path.join(self.output_dir, f'benchmark_detailed_runs_{timestamp}.csv')
    ...
    def _export_strategy_comparison(self, timestamp):
        """Export strategy comparison across paths"""
        csv_path = os.path.join(self.output_dir, f'benchmark_strategy_comparison_{timestamp}.csv')
    ...
    def _export_path_comparison(self, timestamp):
        """Export path comparison across strategies"""
        csv_path = os.path.join(self.output_dir, f'benchmark_path_comparison_{timestamp}.csv')
    ...
    def _export_summary_statistics(self, timestamp):
        """Export summary statistics for each strategy-path combination"""
        csv_path = os.path.join(self.output_dir, f'benchmark_summary_statistics_{timestamp}.csv')
    ...
    def print_console_summary(self):
        ...
        print("\n" + "="*80)
        print("BENCHMARK SUMMARY - DETAILED ANALYSIS")
        print("="*80)
        ...
        for strategy_name in GA_STRATEGIES.keys():
            ...
            for path_name in PATHS:
                ...
                if data:
                    ...
                    print(f"    Fitness:        {data.get('avg_fitness', 0):.6f} ± {data.get('std_fitness', 0):.6f}")
                    print(f"    Range:          [{data.get('min_fitness', 0):.6f}, {data.get('max_fitness', 0):.6f}]")
                    print(f"    Avg Distance:   {data.get('avg_distance', 0):.2f} px")
                    print(f"    Avg Iterations: {data.get('avg_iterations', 0):.1f}")
                    print(f"    Success Rate:   {data.get('success_rate', 0)*100:.1f}%")
                    print(f"    Efficiency:     {data.get('avg_efficiency', 0):.6f}")
                    print(f"    Crash Rate:     {data.get('crash_rate', 0)*100:.1f}%")
        
        print("\n" + "="*80)
        print("STRATEGY EFFECTIVENESS BY PATH")
        print("="*80)
        ...
        for path_name in PATHS:
            ...
            if best_strategy:
                print(f"  [BEST] {best_strategy.upper()}: {best_fitness:.6f}")
...
if __name__ == '__main__':
    ...
    import constants  # Import constants for vehicle simulation
